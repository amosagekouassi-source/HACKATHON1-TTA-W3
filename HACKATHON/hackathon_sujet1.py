...
Original file is located at
    https://colab.research.google.com/drive/1x-YsUGioyfWA7JmibhcrLqXmkb0nkJFR
"""
...
#── 6. Top prénoms générés ─────────────────────────────────────
ax6 = fig.add_subplot(gs[1, 2])
#Extraction du vrai prénom de la colonne NOM (ex: 'Amos' depuis 'Amos_0')
df_clean['PRENOM_BRUT'] = df_clean['NOM'].apply(lambda x: x.split('_')[0] if isinstance(x, str) else x)
top_prenoms = df_clean['PRENOM_BRUT'].value_counts().head(8)
ax6.barh(top_prenoms.index, top_prenoms.values, color='#fb923c', edgecolor='white', linewidth=0.5)
ax6.invert_yaxis()
ax6.set_title("Top 8 des prénoms les plus fréquents", color='white', fontsize=12, fontweight='bold', pad=15)
ax6.grid(axis='x', linestyle='--', alpha=0.2)
